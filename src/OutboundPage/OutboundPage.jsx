...
const OutboundPage = () => {
  ...
  const fetchOutboundDetails = async (itemId, page = 0) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/outbounds/details`, {
        params: { itemId, page, size: 10 },
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="flex h-screen">
            ...
            {outbounds.map((outbound, index) => (
              <tr key={index} className="border">
                ...
                <td className="border p-3">
                  <button onClick={() => fetchOutboundDetails(outbound.item, 0)} className="bg-green-500 text-white px-3 py-1 rounded">
                    📄 Xem
                  </button>
                </td>
              </tr>
      ...
      <div className="flex justify-between items-center mt-4">
        <button 
          onClick={() => fetchOutbounds(currentPage - 1)}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-3/4">
            <h3 className="text-lg font-semibold mb-4">📜 Chi tiết xuất kho (Item: {selectedItem})</h3>
            {details.length === 0 ? (
            ...
            {/* Pagination in Popup */}
            <div className="flex justify-between items-center mt-4">
              <button onClick={() => fetchOutboundDetails(selectedItem, detailPage - 1)} disabled={detailPage === 0} className={`px-4 py-2 rounded ${detailPage === 0 ? "bg-gray-300" : "bg-blue-500 text-white"}`}>⬅ Trang trước</button>
              <span className="text-lg font-medium">Trang {detailPage + 1} / {detailTotalPages}</span>
              <button onClick={() => fetchOutboundDetails(selectedItem, detailPage + 1)} disabled={detailPage >= detailTotalPages - 1} className={`px-4 py-2 rounded ${detailPage >= detailTotalPages - 1 ? "bg-gray-300" : "bg-blue-500 text-white"}`}>Trang sau ➡</button>
            </div>

            <button className="mt-4 px-4 py-2 bg-red-500 text-white rounded" onClick={() => setIsModalOpen(false)}>✖ Đóng</button>
          </div>
        </div>
      )}
    </div>
