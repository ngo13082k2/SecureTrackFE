...
const OrderDetail = () => {
    ...
    const { ref } = useZxing({
        onDecodeResult(result) {
            console.log("Dữ liệu quét được:", result.text); // Debug để kiểm tra dữ liệu
    
            // Điều chỉnh regex để khớp với cả "QR Code:" và "QRCode:"
            const match = result.text.match(/QR\s?Code:\s*([^\s]+)/i);
            if (match) {
                const qrCode = match[1].trim(); // Lấy mã QR và loại bỏ khoảng trắng
    ...
    const handleSubmit = async () => {
        ...
        const requestData = {
            ...
            qrCodes: qrCodes.length > 0 ? qrCodes : [], // Đảm bảo không gửi null
        };
    ...
    return (
        <div style={{ padding: "20px", maxWidth: "500px", margin: "auto" }}>
            <h2>Quét QR Code</h2>
            <video ref={ref} style={{ width: "100%", border: "1px solid black" }} />
